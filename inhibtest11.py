...
class VisualNLPAudioMemoryModel(nn.Module):
    ...
    def forward(self, qualia_seq):
        memory = torch.zeros(self.memory_dim)
        outputs = []

        for x in qualia_seq:
            combined = torch.cat([x, memory])
...
def train_seq2seq_nlp(model, tokenizer, examples, max_len=128, epochs=1000):
    ...
    for epoch in range(epochs + 1):
        ...
        for input_text, output_text in examples:
            in_seq = tokenizer.encode_string(input_text, max_len)
            out_seq = tokenizer.encode_string(output_text, max_len)
...
def train_autoregressive_nlp(model, tokenizer, examples, max_len=128, epochs=1000):
    ...
    for epoch in range(epochs + 1):
        ...
        for input_text, output_text in examples:
            ...
            memory = torch.zeros(model.memory_dim)
        ...
        if epoch % 100 == 0 or epoch == epochs:
            ...
            print("Sample output:")
            decoded = decode_output(pred, tokenizer)
            print(decoded)
# ---- Target labels ----
concepts = {
...
# ---- Main training loop ----
def train_sentence_model_with_memory_attention():
    save_fake_image()

    model = VisualNLPAudioMemoryModel()  # now includes MemoryAttention
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    loss_fn = nn.BCEWithLogitsLoss()
    ...
    qualia_seq = [
        encode_qualia_with_audio(w, img_blocks[i], audio_seed=i)
    ...
    for epoch in range(1001):
        optimizer.zero_grad()
        preds = model(qualia_seq)  # forward call now includes memory-attn
