...
def load_math_json(path, max_samples=100):
    ...
    for entry in data[:max_samples]:
        ...
        input_text = next((c["value"] for c in convs if c["from"] == "human"), None)
        output_text = next((c["value"] for c in convs if c["from"] == "gpt"), None)
        if input_text and output_text:
            examples.append((input_text.strip(), output_text.strip()))
    return examples


def tokenize_nlp_to_qualia(text, tokenizer, domain="text"):
    return [tokenizer.encode(c, domain) for c in text if c.isprintable()]

class NLPQualiaTokenizer:
    def __init__(self):
        self.alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,;!?-+/=()[]{}\\$%&^@#*:\"' \n"
        self.vocab = {ch: i for i, ch in enumerate(self.alphabet)}
        self.domain_tag = torch.tensor([1.0, 0.0])  # fixed for text
    ...
    def encode_string(self, text, max_len=256):
        return torch.stack([self.encode_char(ch) for ch in text[:max_len]])

def plot_memory(memory_snapshots, words):
    memory_snapshots = memory_snapshots.cpu().numpy()
    plt.figure(figsize=(12, 4))
    for i, mem in enumerate(memory_snapshots):
        plt.plot(mem, label=f"memory after '{words[i]}'")
    plt.title("Memory Evolution Across Tokens")
...
# ---- Main model ----
class VisualNLPAudioMemoryModel(nn.Module):
    def __init__(self, qualia_dim=12346, memory_dim=64, hidden_dim=128, out_dim=3):
        super().__init__()
        self.memory_dim = memory_dim
        self.encoder = nn.Linear(qualia_dim + memory_dim, hidden_dim)
        self.inhibitor = InhibitionNeuron(hidden_dim)
        self.decoder = nn.Linear(hidden_dim, out_dim)
        self.memory_attn = MemoryAttention(self.memory_dim)

    def forward(self, qualia_seq):
        memory = torch.zeros(self.memory_dim).to(device)
        outputs = []

        for x in qualia_seq:
            combined = torch.cat([x.to(device), memory])
            h = self.inhibitor(self.encoder(combined))
            out = self.decoder(h)
            outputs.append(out)

            # memory attention modulation
            raw_mem = h[:self.memory_dim]
            memory = self.memory_attn(raw_mem)

        return torch.stack(outputs)


def generate_synthetic_audio_vector(seed, length=32):
    torch.manual_seed(seed)
    t = torch.linspace(0, 1, steps=length)
    base = torch.sin(2 * np.pi * (seed + 1) * t)
...
def extract_visual_blocks(path, cols=3):
    ...
    blocks = []
    for i in range(cols):
        patch = img.crop((i * block_w, 0, (i + 1) * block_w, h))
        patch = patch.resize((64, 64))  # force consistent shape
        patch_tensor = torch.tensor(np.array(patch)).float().view(-1) / 255.0
        blocks.append(patch_tensor)
    return blocks
...
def encode_qualia_with_audio(token, visual_patch, audio_seed):
    alphabet = "abcdefghijklmnopqrstuvwxyz"
    one_hot = torch.zeros(26)
    if token.lower() in alphabet:
        one_hot[alphabet.index(token.lower()[0])] = 1.0
    audio = generate_synthetic_audio_vector(seed=audio_seed, length=32)
    return torch.cat([one_hot, visual_patch, audio])
...
class MemoryAttention(nn.Module):
    def __init__(self, mem_dim):
        super().__init__()
        self.query = nn.Linear(mem_dim, mem_dim)
        self.key = nn.Linear(mem_dim, mem_dim)
        self.value = nn.Linear(mem_dim, mem_dim)

    def forward(self, prev_mem):
        # prev_mem: [mem_dim] → reshape to [1, mem_dim]
        prev_mem = prev_mem.unsqueeze(0)  # shape: [1, mem_dim]
        q = self.query(prev_mem)
        k = self.key(prev_mem)
        v = self.value(prev_mem)

        # Self-attention over singleton: dot(q, k.T) → [1, 1]
        attn_scores = torch.matmul(q, k.transpose(-2, -1)) / (prev_mem.size(-1) ** 0.5)
        attn_weights = F.softmax(attn_scores, dim=-1)  # still [1, 1]
        attended = torch.matmul(attn_weights, v)  # [1, mem_dim]

        return attended.squeeze(0)  # return [mem_dim]

def decode_output(pred_seq, tokenizer):
    # Get the character with highest activation from one-hot
    idx_to_char = {v: k for k, v in tokenizer.vocab.items()}
    pred_chars = []
    for vec in torch.sigmoid(pred_seq):
        ch_idx = vec[:-2].argmax().item()
        pred_chars.append(idx_to_char.get(ch_idx, '?'))
    return ''.join(pred_chars)

def train_seq2seq_nlp(model, tokenizer, examples, max_len=128, epochs=1000):
    ...
    for epoch in range(epochs + 1):
        ...
        for input_text, output_text in examples:
            in_seq = tokenizer.encode_string(input_text, max_len).to(device)
            out_seq = tokenizer.encode_string(output_text, max_len).to(device)


            pred_seq = model(in_seq)  # returns output for each step
            min_len = min(pred_seq.shape[0], out_seq.shape[0])
        ...
        if epoch % 10 == 0 or epoch == epochs:
            print(f"[Seq2Seq Epoch {epoch}] Loss: {total_loss:.6f}")
            print("Sample output:")
            decoded = decode_output(pred_seq, tokenizer)
            print(decoded)

def train_autoregressive_nlp(model, tokenizer, examples, max_len=128, epochs=1000):
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-2)
    loss_fn = nn.BCEWithLogitsLoss()
    ...
    for epoch in range(epochs + 1):
        total_loss = 0
        for input_text, output_text in examples:
            full_seq = tokenizer.encode_string(input_text + output_text, max_len + max_len)
            loss = 0.0

            memory = torch.zeros(model.memory_dim).to(device)
            for i in range(1, full_seq.size(0)):
                x = full_seq[i - 1].to(device)
                target = full_seq[i].to(device)

                combined = torch.cat([x, memory]).to(device)
                h = model.inhibitor(model.encoder(combined))
                pred = model.decoder(h)
                memory = h[:model.memory_dim].detach()

                loss += loss_fn(pred.to(device), target.to(device))

            loss = loss / full_seq.size(0)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if epoch % 10 == 0 or epoch == epochs:
            print(f"[AutoReg Epoch {epoch}] Loss: {total_loss:.6f}")
            print("Sample output:")
            test_inference(model, tokenizer, "Find the slope of the line $3x+5y=20$.")

def test_inference(model, tokenizer, prompt, max_len=128):
    model.eval()
    with torch.no_grad():
        input_vec = tokenizer.encode_string(prompt, max_len).to(next(model.parameters()).device)
        output = model(input_vec)
        decoded = decode_output(output, tokenizer)
...
def run_seq2seq_nlp_training():
    # Load dataset
    examples = load_math_json("C:/Users/abias/.cursor-tutor/vccdoe/mhlamodel/mhla/KANseriesNeuralNetwork-main/inhibitorynetwork/data/mathtest.json", max_samples=10)

    # Initialize tokenizer and model
    tokenizer = NLPQualiaTokenizer()
    input_dim = len(tokenizer.vocab) + 2  # + domain tag
    model = VisualNLPAudioMemoryModel(qualia_dim=input_dim, memory_dim=64, hidden_dim=128, out_dim=input_dim)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)


    # Train on NLP-only qualia
    train_seq2seq_nlp(model, tokenizer, examples, max_len=128, epochs=1000)
    test_inference(model, tokenizer, "Find the slope of the line $3x+5y=20$.")

def run_autoregressive_nlp_training():
    # Load dataset
    examples = load_math_json("C:/Users/abias/.cursor-tutor/vccdoe/mhlamodel/mhla/KANseriesNeuralNetwork-main/inhibitorynetwork/data/mathtest.json", max_samples=10)
    ...
    # Train on NLP-only qualia autoregressively
    train_autoregressive_nlp(model, tokenizer, examples, max_len=128, epochs=1000)
    test_inference(model, tokenizer, "Find the slope of the line $3x+5y=20$.")

if __name__ == "__main__":
    #run_seq2seq_nlp_training()
# or
    run_autoregressive_nlp_training()
