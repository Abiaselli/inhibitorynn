...
class VisualNLPAudioMemoryModel(nn.Module):
    ...
    def forward(self, qualia_seq):
        ...
        outputs = []
        for x in qualia_seq:
            #print(f"x shape: {x.shape}")  # should be [12410]
            #print(f"memory shape: {memory.shape}")  # should be [12410]

            combined = torch.cat([x, memory])
            #print(f"self.encoder.weight shape: {self.encoder.weight.shape}")
            #print(f"Combined shape after torch.cat: {combined.shape}")  # should be [12410]
...
# ---- Main training loop ----
def train_sentence_model_with_audio_memory():
    save_fake_image()
    model = VisualNLPAudioMemoryModel()
...
if __name__ == "__main__":
    train_sentence_model_with_audio_memory()
