...
def auto_transplant_if_needed(model, checkpoint_path="model_weights.pth", strict_modules=("fc1", "fc2", "inhibitor", "encoder", "memory_attn", "input_proj")):
    ...
    try:
        ...
        for key, val in saved_state.items():
            if key in current_state:
                if val.shape == current_state[key].shape:
                    transplantable[key] = val
                    print(f"[=] Loaded '{key}' unchanged.")
                elif all(s <= c for s, c in zip(val.shape, current_state[key].shape)):
                    padded = current_state[key].clone()
                    slices = tuple(slice(0, s) for s in val.shape)
                    padded[slices] = val
                    transplantable[key] = padded
                    print(f"[~] Padded '{key}' from {val.shape} → {current_state[key].shape}")
                else:
                    print(f"[x] Skipped '{key}' (saved: {val.shape}, current: {current_state[key].shape})")
            else:
                print(f"[x] Key '{key}' not found in model.")

        model.load_state_dict(transplantable, strict=False)
...
class InhibitorySeq2SeqModel(nn.Module):
    def __init__(self, input_dim, output_dim, qualia_dim=QUALIA_DIM, hidden_dim=HIDDEN_DIM, memory_dim=MEMORY_DIM):
        super().__init__()
        self.input_proj = nn.Linear(output_dim, QUALIA_DIM)
        self.encoder = nn.Linear(HIDDEN_DIM, HIDDEN_DIM)
        self.inhibitor = nn.Linear(HIDDEN_DIM, HIDDEN_DIM)
        self.memory_attn = nn.MultiheadAttention(HIDDEN_DIM, num_heads=4, batch_first=True)
    ...
    def forward(self, x):
        ...
        inh = self.inhibitor(encoded)
        mem, _ = self.memory_attn(inh, inh, inh)
        out = self.fc1(mem)
        out= self.fc2(out)
    ...
    def get_core_weights(self):
        return {
            ...
            "inhibitor.weight": self.inhibitor.weight,
    ...
    def load_core_weights(self, state_dict):
        self.encoder.load_state_dict(state_dict['encoder.weight'])
        self.inhibitor.load_state_dict(state_dict['inhibitor.weight'])
        self.fc1.load_state_dict(state_dict['fc1.weight'])
        self.fc2.load_state_dict(state_dict['fc2.weight'])
        self.input_proj.load_state_dict(state_dict['input_proj.weight'])
...
def run_seq2seq_nlp_training():
    #path = r"C:\Users\abias\alldatareasoning\iimath\1\infinityinstructmath1.json"
